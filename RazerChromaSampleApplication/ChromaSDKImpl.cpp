...
DWORD WINAPI Thread_AnimateAlphabetOnKeyboard(LPVOID lpParameter)
{
	Keyboard::CUSTOM_KEY_EFFECT_TYPE Effect = {};
	...
	// Fill up the key effects.
	for (UINT i : RazerKeys) {
		COLORREF Color = 0x01000000 | ActiveKeysData.Color;
		Effect.Key[HIBYTE(i)][LOBYTE(i)] = Color;
		CreateKeyboardEffect(Keyboard::CHROMA_CUSTOM_KEY, &Effect, NULL);
		Sleep(750);
	}
...
void CChromaSDKImpl::ShowAlphabet(UINT DeviceType, COLORREF Color) {
	ActiveKeysData.Color = Color;
	HANDLE hThread = CreateThread(NULL, 0, Thread_AnimateAlphabetOnKeyboard, NULL, 0, NULL);
